...
public class ConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.filterButton = this.addRenderableWidget(
                Button.builder(Component.literal("Manage Items..."), button -> {
                    ModConfig.save();
                    this.minecraft.gui.setScreen(new ItemFilterScreen(this));
                })
        ...
        this.mainOverrideButton = this.addRenderableWidget(
                Button.builder(Component.literal("Select Pack..."), button -> {
                    ModConfig.save();
                    this.minecraft.gui.setScreen(new MainOverridePackScreen(this));
                })
        ...
        this.basePackButton = this.addRenderableWidget(
                Button.builder(Component.literal("Select Pack..."), button -> {
                    ModConfig.save();
                    this.minecraft.gui.setScreen(new BaseResourcePackScreen(this));
                })
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.centeredText(this.font, "§f§lZap's Model Only Resources §7(ZMOR)", this.width / 2, 4, 0xFFFFFFFF);
        graphics.centeredText(this.font, "§7Configure selective custom resource pack texture rendering", this.width / 2, 14, 0xFFAAAAAA);

        // Top Summary Bar
        int summaryY = 24;
        graphics.fill(startX, summaryY, startX + panelWidth, summaryY + 16, 0x40000000);
        graphics.outline(startX, summaryY, panelWidth, 16, 0x20FFFFFF);
        ...
        graphics.text(this.font, "§7Override: §e" + overrideDisplay, startX + 8, summaryY + 4, 0xFFFFFFFF);
        graphics.text(this.font, "§7Base: §e" + baseDisplay, startX + 138, summaryY + 4, 0xFFFFFFFF);
        String countStr = "§7Whitelist: §b" + filterCount;
        graphics.text(this.font, countStr, startX + panelWidth - this.font.width(countStr) - 8, summaryY + 4, 0xFFFFFFFF);
        ...
        graphics.outline(startX, section1Y, panelWidth, section1Height, 0x20FFFFFF);

        graphics.text(this.font, "§f§lMultiplayer & Visibility Scopes", startX + 10, section1Y + 4, 0xFFFFFFFF);

        int rowSpacing = 24;
        // Row 1
        graphics.text(this.font, "Other Players", startX + 12, section1Y + 15, 0xFFFFFFFF);
        graphics.text(this.font, "§7Show your custom textures on other players", startX + 12, section1Y + 25, 0xFFAAAAAA);

        // Row 2
        graphics.text(this.font, "Mobs & Armor Stands", startX + 12, section1Y + 15 + rowSpacing, 0xFFFFFFFF);
        graphics.text(this.font, "§7Show custom armor on mobs & armor stands", startX + 12, section1Y + 25 + rowSpacing, 0xFFAAAAAA);

        // Row 3
        graphics.text(this.font, "Item Frames", startX + 12, section1Y + 15 + rowSpacing * 2, 0xFFFFFFFF);
        graphics.text(this.font, "§7Show custom item textures in item frames", startX + 12, section1Y + 25 + rowSpacing * 2, 0xFFAAAAAA);

        // Row 4
        int row4Y = section1Y + 15 + rowSpacing * 3;
        graphics.text(this.font, "Sync Peer Packs §e[?]", startX + 12, row4Y, 0xFFFFFFFF);
        graphics.text(this.font, "§7Download & view custom packs from other players", startX + 12, row4Y + 10, 0xFFAAAAAA);
        ...
        graphics.outline(startX, section2Y, panelWidth, section2Height, 0x20FFFFFF);

        graphics.text(this.font, "§f§lResource Overrides & Whitelist", startX + 10, section2Y + 4, 0xFFFFFFFF);

        // Row 1
        graphics.text(this.font, "Item Whitelist Filter", startX + 12, section2Y + 15, 0xFFFFFFFF);
        graphics.text(this.font, "§7" + (filterCount == 0 ? "No items whitelisted (Shows custom for all)" : filterCount + " item" + (filterCount == 1 ? "" : "s") + " isolated"), startX + 12, section2Y + 25, 0xFFAAAAAA);

        // Row 2
        graphics.text(this.font, "Main Override Pack", startX + 12, section2Y + 15 + rowSpacing, 0xFFFFFFFF);
        graphics.text(this.font, "§7Primary custom textures source", startX + 12, section2Y + 25 + rowSpacing, 0xFFAAAAAA);

        // Row 3
        graphics.text(this.font, "Fallback Base Pack", startX + 12, section2Y + 15 + rowSpacing * 2, 0xFFFFFFFF);
        graphics.text(this.font, "§7Fallback texture provider for peers/mobs", startX + 12, section2Y + 25 + rowSpacing * 2, 0xFFAAAAAA);
